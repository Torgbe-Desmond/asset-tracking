...
{
    public record GetAllAssetsDetailsQuery() : IRequest<List<AssetDetailDto>>;
    public class GetAllAssetsDetailsHandler(IAssetRepository assetRepository) : IRequestHandler<GetAllAssetsDetailsQuery, List<AssetDetailDto>>
    {

        public async Task<List<AssetDetailDto>> Handle(
            GetAllAssetsDetailsQuery request,
            CancellationToken cancellationToken)
        {
           
            var assets = await assetRepository.GetAllAsync(cancellationToken);

            var dtos = assets.Select(asset => new AssetDetailDto
            {
